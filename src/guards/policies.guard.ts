...
import { IPolicyClass, POLICY_KEY, ACTION_KEY, RESOURCE_ENTITY_KEY } from "decorators/policy.decorator";
...
export class PoliciesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext)  {
    ...
    const resource = await this.dataSource.getRepository(resourceClass).findOneBy({ id: params?.id })
