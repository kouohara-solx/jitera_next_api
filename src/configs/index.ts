...
export default (): Config => {
  ...
  return {
      ...
      disks: {
        ...
        s3: {
          ...
          config: {
            ...
            endpoint: process.env.S3_ENDPOINT || credentials?.aws?.s3_endpoint || `https://s3.${process.env.AWS_DEFAULT_REGION || 'ap-northeast-1'}.amazonaws.com`,
    ...
    jwt: {
      accessSecret: process.env.JWT_ACCESS_SECRET || credentials?.jwt?.access_token_secret || 'secret',
      refreshSecret: process.env.JWT_REFRESH_SECRET || credentials?.jwt?.refresh_token_secret || 'secret',
      expiresIn: process.env.JWT_EXPIRES_IN || '7d',
      refreshIn: process.env.JWT_REFRESH_IN || '30d',
      bcryptSaltOrRound: 10,
    },
    authentication: {
      sendConfirmationEmail: Boolean(process.env.AUTH_SEND_CONFIRMATION_EMAIL) || true,
      ...
      resetPasswordIn: +process.env.AUTH_RESET_PASSWORD_IN || 1,
          ...
          unlockIn: '6h',
      unlockStrategy: 'time',
      unlockAccessUrl: process.env.UNLOCK_ACCESS_URL || 'http://localhost:3000/unlock',
    ...
    twilio: {
      ...
      verificationServiceSid: process.env.TWILIO_VERIFICATION_SERVICE_SID  || credentials?.twilioLogin?.serviceId,
    },
    deepLink: {
      ...
      androidPackageName: process.env.ANDROID_PACKAGE_NAME || credentials?.deepLink?.androidPackageName,
      androidAppLink: process.env.ANDROID_APP_LINK || credentials?.deepLink?.androidPackageName,      
      androidCertFingerprints: process.env.ANDROID_CERT_FINGERPRINTS || credentials?.deepLink?.androidCertFingerprints,
    ...
    line: {
      ...
      clientSecret: process.env.LINE_CLIENT_SECRET || credentials?.line?.secretKey || 'clientSecret',
    },
    rollbar: {
